...
class rssMBEngine {
	...
	public function import_feed() {
		...
		// update options
		update_option('rss_mb_feeds', array(
			'feeds' => $this->options['feeds'],
			'settings' => $this->options['settings'],
	...
	private function insert($items, $args = array()) {
		...
		$parser = new rssMBParser($this->options);
...
//		} else {
		if ( ! $post_exists ) {
			...
			if ($posts = $wpdb->get_results("SELECT ID FROM " . $wpdb->prefix . "posts WHERE post_title = '" . $title . "' and post_status = 'publish' ", 'ARRAY_A')) {
				...
				foreach ($posts as $post) {
					$post_id = $post['ID'];
					$source_url = get_post_meta($post_id, 'rss_mb_source_url', true);
		...
		if ( ! $post_exists && $this->options['settings']['cache_deleted'] == 'true' ) {
			...
			if ( $this->options['upgraded']['deleted_posts'] ) { // database migrated
				...
				if ( in_array( $permalink_md5, $rss_mb_deleted_posts ) || in_array( $permalink_md5_new, $rss_mb_deleted_posts ) ) {
					$post_exists = TRUE;
				}
			} else {
				//do it the old fashion way
				$rss_mb_imported_posts = get_option( 'rss_mb_imported_posts', array() );
				if ( in_array( $permalink, $rss_mb_imported_posts ) ) {
					$post_exists = TRUE;
				}
			}
		}
	...
	private function _insert($post, $url) {
		...
		$_post = apply_filters('pre_rss_mb_insert_post', $post);

		$post_id = wp_insert_post($_post);

		add_action('save_rss_mb_post', $post_id);
