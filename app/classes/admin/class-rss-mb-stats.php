...
if (!class_exists("Rss_mb_stats")) {
	...
	class Rss_mb_stats {
		...
		function show_charts() {
			...
			$feeds = get_option("rss_mb_feeds", array());

			$oldest_post = $this->get_the_oldest_post();
			if ($oldest_post == false) {
//				echo 'No Stats';
				?>
				<div class="rss_mb_stat_message">Sorry, there are no imported posts to show stats for.</div>
...
//				echo 'Dates problem';
				?>
				<div class="rss_mb_stat_message">Please enter correct dates.</div>
				<script type="text/javascript">
					function drawChart() {}
				</script>
...
//				return;
				?>
				<div class="rss_mb_stat_message">Range too long, reduced to a year ago.</div>
				<?php
				$start_time = $end_time - 365 * 24 * 60 * 60;
			}
			if ( isset($_POST["rss_filter_stats"]) && $start_time > $end_time ) {
				?>
				<div class="rss_mb_stat_message">From date were after the Till date, reversed.</div>
				<?php
...
<?php
		...
		function draw_line_charts_js($feeds_data, $feeds) {
			...
			var chart = new google.visualization.LineChart(document.getElementById('rssmb_chart_line'));
		...
		function draw_pie_chart_js($pie_feeds_data, $feeds) {
			...
			var chart = new google.visualization.PieChart(document.getElementById('rssmb_chart_pie'));
		...
		function draw_bar_chart_js($bar_feeds_data, $feeds) {
			...
			var bar_chart = new google.charts.Bar(document.getElementById('rssmb_chart_bar'));
